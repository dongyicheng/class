...
let phoneFn=function () {
    ...
    $listenBtn.tap(function () {
        ...
        let sayTimer=setInterval(function () {
            ...
            $timeBox.html(str);
            //需要把say.currentTimer  转成  00:34  这个格式的字符串

            //需要判断音频是否播放完毕   若播放完毕 , 则直接执行 挂机键执行的操作
            if(say.ended){
        ...
        function phoneEnd() {
            let h = document.documentElement.clientHeight || document.documentElement.clientHeight ;
            //获取到的是个以  px 为单位的数字
            $phoneBox.css({
...
let msgFn=function () {
  //让每一个 li 先都透明  并且下移
    ...
    function inputWord() {
        ...
        let str2='';//当前要显示的字体内容
        let timer2=null;
        let index=0;//控制当前要出来的字

        //让字体出现完成之后  去让按钮显示

        timer2=setInterval(function () {
            if(index==str.length) {//说明字已经全部显示了
                ...
                $btn.tap(function () {
                    ...
                    move();
                }) //点击按钮  要做 1.清空文本框  2.让键盘下去  3.对话框接着弹
                return;
            }
            str2+=str[index];
            $textBox.html(str2);//让str2的内容显示在文本框
            index++;
        },200)
    }
};