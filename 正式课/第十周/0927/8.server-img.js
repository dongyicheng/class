...
// 客户端可以通过地址栏在有网的情况下可以向服务器发送请求，首先经过DNS域名进行解析，找到对应的IP地址那个服务器；服务器很久url中的端口号找到自己端口下的项目；根据url中的路径进行读数据；读取成功后，发送给客户端之前，会首先将响应头发送给客户端；然后把数据整理发送给客户端；客户端接收到数据，形成DOM树，css树，render树进行渲染；
